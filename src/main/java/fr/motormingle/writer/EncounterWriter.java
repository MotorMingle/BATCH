...
public class EncounterWriter implements ItemWriter<List<Encounter>> {
    ...
    @Override
    public void write(@NotNull Chunk<? extends List<Encounter>> encounters) throws Exception {
        JdbcTemplate jdbcTemplate = new JdbcTemplate(datasource);
        encounters.getItems().get(0).forEach((encounter) -> {
            jdbcTemplate.update("INSERT INTO encounter(hash, count, date, status, user_id_1, user_id_2) " +
                            "VALUES (?, ?, ?, ?, ?, ?) " +
